...
- 설비 이상 이벤트가 발생하면 **Kafka 토픽**을 통해 원천 이벤트를 실시간으로 수신하고, 이상 대응 가이드 생성 프로세스를 자동으로 실행
- 백엔드에서 이벤트의 **위험도와 우선순위**를 분석하여 가장 시급한 알림을 선별하고, 담당자의 숙련도에 따라 **Junior·Senior 맞춤형 가이드**를 요청
- 설비 매뉴얼, 이벤트 이력, 점검·유지보수 이력, 공정·설비 정보를 **벡터 데이터베이스**에 저장하고 RAG 검색에 활용
- **멀티프로세싱 기반 RAG 파이프라인**을 적용하여 여러 데이터 소스를 병렬로 검색하고, 관련성이 높은 정보를 신속하게 추출
- 검색된 문서와 이상 이벤트 정보를 조합해 **LLM이 단계별 조치 방법, 체크리스트 및 권장 대응 방안**을 생성
- 생성된 맞춤형 대응 가이드를 **React 대시보드**에 즉시 제공하여 담당자가 이상 상황에 빠르게 대응할 수 있도록 구성
...
- 관계형 데이터베이스의 복잡한 조건 조회 부담을 줄이고, 대량의 제조 데이터 환경에서도 **빠른 응답 속도**를 확보
- 시간대별 이벤트 발생량과 이상 탐지 내역을 조회하여 공정 상태와 이상 발생 추이를 실시간으로 분석
